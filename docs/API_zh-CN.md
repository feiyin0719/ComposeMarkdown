...
`MarkdownText` 支持相同参数和行为。parser 通过
`MarkdownRenderConfig.Builder.streamingMarkdownParserFactory` 统一配置，每个组件都会得到独立的
...
Compose 可能将解析结果视为未变化。同时应保持已经完成且未变化的顶层 child block 对象身份不变，
...
- base ID 只需传入节点 class name。helper 会用 occurrence 后缀保证每个 annotation 唯一，因此无需再拼接内容、URL、公式、hash 或源码位置。仅使用类名可缩短 ID、避免暴露内容，并将 ID 生成规则集中在一处。
